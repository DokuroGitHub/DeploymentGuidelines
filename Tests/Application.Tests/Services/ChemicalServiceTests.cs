...
public class ChemicalServiceTests : SetupTest
{
    ...
    [Fact]
    public async Task GetPagedListChemicalsAsync_ShouldReturnCorrectDataWhenDidNotPassTheParameters()
    {
        //arrange
        var mockData = new PagedList<Chemical>
        {
        ...
        var expectedResult = _mapperConfig.Map<PagedList<Chemical>>(mockData);

        _unitOfWorkMock.Setup(x => x.ChemicalRepository.ToPagination(0, 10)).ReturnsAsync(mockData);

        //act
        var result = await _chemicalService.GetPagedListChemicalsAsync();
